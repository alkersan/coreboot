...
static void configure_ec_gpio(void)
{
	...
	if (CONFIG(DISABLE_WWAN_GBE_MAPLE)) {
		tmp &= ~EC9_WWAN_RST_N;
	} else {
		tmp |= EC9_WWAN_RST_N;
	}
	...
	if (CONFIG(DISABLE_WWAN_GBE_MAPLE)) {	// no WWAN, turn off WWAN power
		tmp &= ~ECE_WWAN_PWR_EN;
	} else {
		tmp |= ECE_WWAN_PWR_EN;
	}

	if (CONFIG(DISABLE_WLAN_SD_MAPLE)) {	// no WLAN, turn off WLAN power
		tmp &= ~ECE_WLAN_PWR_EN;
	} else {
		tmp |= ECE_WLAN_PWR_EN;
	}
