...
#define MAPLE_EC_CMD	0x666
#define MAPLE_EC_DATA	0x662
...
#define   ECE_GBE_N_FPR_SEL     BIT(1) // USB6_MUX_GBE#_FPR_SEL for Maple
#define   ECE_TPNL_N_BT_SEL     BIT(0) // USB5_MUX_TPNL#_BT_SEL for Maple

#define EC_GPIO_F_ADDR		0xAF
#define   ECF_SD_MAIN_PWR_EN	       BIT(6)
#define   ECF_WLAN_LANE0_WWAN_LANE1_SW BIT(1)
#define   ECF_WWAN_LANE0_WLAN_LANE1_SW BIT(0)
...
static void configure_ec_gpio(void)
{
	...
	tmp |= EC3_DT_RST_AUX | EC3_LOM_RST_AUX;
	...
	tmp &= ~(EC7_BT_RADIO_DIS | EC7_BT_RADIO_DIS); // todo: typo?
	printk(BIOS_SPEW, "Write reg [0x%02x] = 0x%02x\n", EC_GPIO_7_ADDR, tmp);
	...
	tmp |= ECE_SSD0_PWR_EN | ECE_CAM_PWR_EN | ECE_GBE_N_FPR_SEL;
	tmp &= ~ECE_TPNL_N_BT_SEL;
	...
	if (CONFIG(ENABLE_WLANx1_WWANx1_MAPLE)) {
		tmp &= ~ECF_WWAN_LANE0_WLAN_LANE1_SW;
		tmp &= ~ECF_WLAN_LANE0_WWAN_LANE1_SW;
	}
	...
	tmp |= ECG_U0_WLAN_HDR_SEL | ECG_NFC_BUF_EN;
	printk(BIOS_SPEW, "Write reg [0x%02x] = 0x%02x\n", EC_GPIO_G_ADDR, tmp);
	ec_write(EC_GPIO_G_ADDR, tmp);
}

void maple_ec_init(void)
{
	ec_set_ports(MAPLE_EC_CMD, MAPLE_EC_DATA);
	configure_ec_gpio();
}
