...
#endif
#if CONFIG_MAX_ROOT_PORTS > 12
		{ PCI_DEVFN_PCIE13,	ELOG_WAKE_SOURCE_PME_PCIE13 },
		{ PCI_DEVFN_PCIE14,	ELOG_WAKE_SOURCE_PME_PCIE14 },
#endif
	};

	for (size_t i = 0; i < ARRAY_SIZE(pme_map); i++) {
...
static void pch_log_pme_internal_wake_source(void)
{
	const struct pme_map ipme_map[] = {
		...
		{ PCI_DEVFN_CNVI_BT,	ELOG_WAKE_SOURCE_PME_BLUETOOTH },
	...
	for (size_t i = 0; i < ARRAY_SIZE(ipme_map); i++) {
	...
	for (size_t i = 0; i < NUM_TBT_FUNCTIONS; i++) {
	...
	/* Check DMA devices */
	for (size_t i = 0; i < NUM_TCSS_DMA_FUNCTIONS; i++) {
...
static void pch_log_wake_source(struct chipset_power_state *ps)
{
	...
	/* SMBUS Wake */
	if (CONFIG(SOC_INTEL_COMMON_FEATURE_ELOG_SMBUS_WAKE) &&
	    (ps->gpe0_sts[GPE_STD] & SMB_WAK_STS))
