...
{
	const struct soc_intel_novalake_config *config = config_of_soc();
...
static void soc_pmc_fill_ssdt(const struct device *dev)
{
	...
	acpigen_write_name_string("_DDN", "Intel(R) Nova Lake IPC Controller");
	...
	if (CONFIG(SOC_INTEL_COMMON_BLOCK_ACPI_PEP)) {
		const struct soc_pmc_lpm nvl_pmc_lpm = {
			.num_substates = 8,
			.num_req_regs = 6,
		...
		generate_acpi_power_engine_with_lpm(&nvl_pmc_lpm);
	}
...
static void soc_pmc_init(struct device *dev)
{
	 ...
	 * NVL has its PMC device available for device_operations, it can be
