...
static const pci_devfn_t i3c_dev[] = {
	PCI_DEVFN_I3C1,
	PCI_DEVFN_I3C2,
};
...
__weak void mainboard_update_soc_chip_config(struct soc_intel_novalake_config *config)
{
...
static const struct slot_irq_constraints irq_constraints[] = {
	...
	},
	{
		...
		.fns = {
			...
			ANY_PIRQ(PCI_DEVFN(PCI_DEV_SLOT_ESPI, 4)),
...
static void fill_fsps_lpss_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
	...
	for (i = 0; i < CONFIG_SOC_INTEL_I3C_DEV_MAX; i++)
		s_cfg->SerialIoI3cMode[i] =
			is_devfn_enabled(i3c_dev[i]) ? config->serial_io_i3c_mode[i] : 0;
...
static void fill_fsps_cpu_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
...
static void fill_fsps_microcode_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
...
static void fill_fsps_igd_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
	size_t vbt_len;

	/* Load VBT before devicetree-specific config. */
	s_cfg->GraphicsConfigPtr = (uintptr_t)locate_vbt(&vbt_len);
	s_cfg->VbtSize = vbt_len;
...
static void fill_fsps_chipset_lockdown_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
	/* Chipset Lockdown */
...
static void fill_fsps_xhci_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
	size_t i;
	...
	for (i = 0; i < CONFIG_SOC_INTEL_USB2_DEV_MAX; i++) {
		...
		s_cfg->PortUsb20IntegratedCamera[i] = config->usb2_camera_ports[i];
	}
...
static void fill_fsps_xdci_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
	s_cfg->XdciEnable = xdci_can_enable(PCI_DEVFN_USBOTG);
}

static void fill_fsps_thermal_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
...
static void fill_fsps_irq_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
 ...
 */
static void fill_fsps_pci_ssid_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
...
static void fill_fsps_lan_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
	s_cfg->PchLanEnable = is_devfn_enabled(PCI_DEVFN_GBE);
}

static void fill_fsps_cnvi_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
...
static void fill_fsps_vmd_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
	s_cfg->VmdEnable = is_devfn_enabled(PCI_DEVFN_VMD);
}

static void fill_fsps_pmcpd_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
	s_cfg->PmcPdEnable = 1;
}

static void fill_fsps_thc_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
...
static void fill_fsps_8254_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
...
static void fill_fsps_pm_timer_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
	/*
...
static void fill_fsps_pcie_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
...
static void fill_fsps_misc_power_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
	/* Skip setting D0I3 bit for all HECI devices */
	s_cfg->DisableD0I3SettingForHeci = 1;

	/* Disable SSE from PCI config space */
	s_cfg->SseCommunication = 2;
...
static void fill_fsps_npu_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
	s_cfg->NpuEnable = is_devfn_enabled(PCI_DEVFN_NPU);
}

static void fill_fsps_audio_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
...
static void fill_fsps_iax_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
	s_cfg->IaxEnable = is_devfn_enabled(PCI_DEVFN_IAA);
}

static void fill_fsps_ufs_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
...
static void soc_silicon_init_params(FSP_S_CONFIG *s_cfg, const config_t *config)
{
	void (*fill_fsps_params[])(FSP_S_CONFIG *s_cfg, const config_t *config) = {
