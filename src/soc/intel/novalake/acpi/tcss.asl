...
#define MCHBAR_TCSS_PCODE_MAILBOX			0x5da0
...
Scope (\_SB.PCI0)
{
	...
	{
		Name (_HID, "INTC107A")
		Name (_DDN, "Intel(R) Nova Lake Input Output Manager(IOM) driver")
	...
	OperationRegion (PBAR, SystemMemory, (GMHB() + MCHBAR_TCSS_PCODE_MAILBOX), 0x08)
	...
	Method (TG0F, 0)
	{
		If (DME0 == 1) {
			If (\_SB.PCI0.TDM0.STAT == 1) {
				...
				Printf("TG0F: TDM0 D3 cold enter")
				\_SB.PCI0.TDM0.D3CE()  /* Enable DMA RTD3 */

				Printf("Push TBT RPs to D3Cold together")
				If (\_SB.PCI0.TRP0.VDID != 0xFFFFFFFF) {
					/* Put RP0 to D3 cold. */
					Printf("TG0F: RP0 D3 cold enter")
					\_SB.PCI0.TRP0.D3CE()
				}
				If (\_SB.PCI0.TRP1.VDID != 0xFFFFFFFF) {
					/* Put RP1 to D3 cold. */
					Printf("TG0F: RP1 D3 cold enter")
					\_SB.PCI0.TRP1.D3CE()
				}
				If (\_SB.PCI0.TRP2.VDID != 0xFFFFFFFF) {
					/* Put RP2 to D3 cold. */
					Printf("TG0F: RP2 D3 cold enter")
					\_SB.PCI0.TRP2.D3CE()
				}
			}
		}
	...
	Method (TCON, 0)
	{
		...
		If (TD3C == 1)  /* It was in D3 cold before. */
		{
			...
			If (Local0 == TCSS_IOM_ACK_TIMEOUT_IN_MS) {
				...
				TD3C = 1    /* Failed, revert back. */
			}
			Else
			{
				Printf("TCON: TCSS D3 exit.");
			}
		}
		Else {
	...
	Method (TCOF, 0)
	{
		...
		{
			Printf("TCOF: Skip D3C entry.")
			Return
		}

		/* Set IOM D3 cold bit if it is in D0 now. */
		If (TD3C == 0)  /* It was in D3 cold exit (D0) before. */
		{
			/* Set IOM D3 cold bit. */
			TD3C = 1    /* Request IOM for D3 cold enter sequence. */
			Local0 = 0  /* Time check counter variable */
			/* Wait for ack, the maximum wait time for the ack is 100 msec. */
			While ((TACK == 0) && (Local0 < TCSS_IOM_ACK_TIMEOUT_IN_MS)) {
				/*
				 * Wait in this loop until TACK becomes 1 with timeout
				 * TCSS_IOM_ACK_TIMEOUT_IN_MS by default.
				 */
				Sleep (1)  /* Delay of 1ms. */
				Local0++
			...
			If (Local0 == TCSS_IOM_ACK_TIMEOUT_IN_MS) {
				...
				TD3C = 0    /* Failed, revert back. */
			}
			Else
			{
				Printf("PowerResource D3C => TCOF TACK=1 TCSS D3 Cold Enter.");
			}
		}
		Else {
			Printf("Drop TCOF as it is already in D3 Cold.")
		}
	}
	...
	Device (TDM0)
	{
		...
		Method (_STA, 0x0, NotSerialized)
		{
			Printf ("TDM0._STA: DME0:%o", ToHexString(DME0))
			If (DME0 == 1) {
				Return (0x0F)
			} Else {
				Return (0x0)
